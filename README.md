...
AWS 表記でやり取りしたい場合や、可動範囲を規格側の値で与えたい場合に使います。
...
**投影角 w / t には専用のハンドルはありません。**θ と φ で工具軸が一意に決まるので、
投影角の円弧は冗長でした。ドラッグすると θ と φ が結合して動くため操作の見通しも悪く、
ハンドルが重なって誤爆する原因にもなっていたので削除しています。
`workConvention` / `travelConvention` は**可動範囲の定義としては残っています** —
方位ごとに許される傾斜角の上限を決めるのに使われます。
